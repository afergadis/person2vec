...
def _get_claim_entity(person_dict, claim_type):
    return DECODER[person_dict['claims'][DECODER[claim_type]][0]['mainsnak']
                   ['datavalue']['value']['id']]
...
def _get_person_attributes(person_dict):
    ...
    attributes_dict.update({
        "description":
        wiki_extract.get_description(person_dict)
    })
    attributes_dict.update({"gender": wiki_extract.get_gender(person_dict)})
    attributes_dict.update({
        "occupation":
        wiki_extract.get_occupation(person_dict)
    })
    attributes_dict.update({
        "birth_date":
        wiki_extract.get_birth_date(person_dict)
    })
    attributes_dict.update({
        "political_party":
...
def main():
    ...
    if fail_counter + success_counter == attempt_counter:
        ...
        print(
            "Fails and successes didn't add up to attempts.. something fishy")
